...
def generate_sample_data(path="data/behaviour_data.csv", n=500):
    ...
    data = {
        ...
        "purchase_count": np.where(
            segments == 0,
            np.random.randint(0, 2, n),
            np.where(
                segments == 1, np.random.randint(2, 8, n), np.random.randint(8, 20, n)
            ),
        ),
        "page_views": np.where(
            segments == 0,
        ...
        "cart_additions": np.where(
            ...
            np.where(
                segments == 1, np.random.randint(3, 10, n), np.random.randint(10, 25, n)
            ),
        ),
