...
def transform_uci_online_retail(
    input_path: Path, output_path: Path, n_users: int = 5000
) -> pd.DataFrame:
    """
    ...
    features_df["total_activity"] = (
        features_df["clicks"]
        + features_df["purchase_count"]
        + features_df["page_views"]
    )
    features_df = features_df.sort_values("total_activity", ascending=False).head(
        n_users
    )
    ...
    cluster_means = (
        features_df.groupby("cluster_raw")["purchase_count"]
        .mean()
...
def load_and_validate_dataset(input_path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"Missing required columns: {missing}. Please ensure your dataset "
    ...
    for col in ["clicks", "purchase_count", "page_views", "cart_additions"]:
        transformed[col] = (
            pd.to_numeric(transformed[col], errors="coerce").fillna(0).astype(int)
        )
        transformed[col] = transformed[col].clip(lower=0)

    transformed["time_spent"] = (
        pd.to_numeric(transformed["time_spent"], errors="coerce")
        .fillna(1)
        .astype(float)
    )
    transformed["time_spent"] = transformed["time_spent"].clip(
        lower=0, upper=1440
    )  # Max 24 hours
    ...
    if "customer_segment" not in transformed.columns:
        ...
        X = transformed[
            ["clicks", "time_spent", "purchase_count", "page_views", "cart_additions"]
        ].values
        X_scaled = StandardScaler().fit_transform(X)
        kmeans = KMeans(n_clusters=3, random_state=42, n_init=10)
        clusters = kmeans.fit_predict(X_scaled)
        ...
        cluster_means = (
            transformed.groupby("cluster_raw")["purchase_count"]
            .mean()
            .sort_values(ascending=False)
        )
        cluster_mapping = {
        ...
        transformed["customer_segment"] = transformed["cluster_raw"].map(
            cluster_mapping
        )
        ...
        transformed["customer_segment"] = pd.to_numeric(
            transformed["customer_segment"], errors="coerce"
        )
    ...
    if "month" in transformed.columns:
        ...
        if invalid_mask.any():
            print(
                f"  Warning: {invalid_mask.sum()} invalid month values, "
                "replacing with 'Jan'"
            )
    ...
    # Return cleaned DataFrame (caller is responsible for persisting it)
    return transformed
...
def main():
    ...
    if args.input:
        ...
        try:
            ...
            df = transform_uci_online_retail(
                raw_path, OUTPUT_FILE, n_users=args.n_users
            )
        except Exception as e:
